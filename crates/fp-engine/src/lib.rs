...
";
    ...
    /// T056 regression (AC1): a human-controlled evilken pressing punch must NOT
    /// get permanently stuck. Before the fix, evilken's ground punch (state 206)
    /// ran a *finite looping* attack animation (anim 206, durations [2,4,4],
    /// `loopstart = 0`), and the state exits on `trigger1 = AnimTime = 0`. The
    /// animation advance skipped the wrap tick's `AnimTime = 0` entirely (jumping
    /// from a negative value straight into the next cycle), so the exit trigger
    /// never fired and the punch looped forever with `ctrl = false` — the "punch
    /// gets stuck, control never restored" bug. After the
    /// [`advance_animation`](fp_character) wrap-tick fix, `AnimTime` reaches `0`
    /// once per loop and the punch completes back to a controllable state.
    ///
    /// Asset-gated: skips cleanly when `test-assets/evilken/` is absent.
    #[test]
    fn evilken_human_punch_is_not_stuck_after_animation_loops() {
        let Some(mut m) = build_evilken_match() else {
            return; // fixture absent -> skip cleanly
        };
        evilken_into_fight(&mut m);
        // Put P1 into a clean, controllable neutral stand.
        {
            let c = &mut m.p1.character;
            c.state_no = 0;
            c.state_type = StateType::Standing;
        ...
        // Press a punch button for one frame, then hold neutral. Within the punch
        // animation's length plus a generous margin the move MUST end and restore
        // control — never loop indefinitely.
        let punch = MatchInput {
            x: true,
            ..MatchInput::none()
        };
        m.tick(punch, MatchInput::none());
        let entered_attack =
            m.p1().character.move_type == MoveType::Attack || m.p1().character.state_no != 0;
        // "Un-stuck" = the punch state ends and hands control back to the neutral
        // stand (state 0 with ctrl). The lingering `move_type` flag is irrelevant
        // to controllability; returning to the controllable neutral state is the
        // real signal that the looping attack animation's `AnimTime = 0` exit
        // fired (before the fix it never did and P1 looped state 206 forever with
        // `ctrl = false`).
        let mut regained_control = false;
        for _ in 0..90 {
            m.tick(MatchInput::none(), MatchInput::none());
            let c = &m.p1().character;
            if c.ctrl && c.state_no == 0 {
        ...
        assert!(
            entered_attack,
            "pressing punch should have left the neutral stand (it did not, so this \
             test cannot assert the un-stick); state={}",
            m.p1().character.state_no
        );
        assert!(
            regained_control,
            "evilken's punch never ended: after 90 idle ticks P1 is still stuck \
             (state={}, ctrl={}, move_type={:?}) — the finite-looping attack \
             animation's `AnimTime = 0` exit trigger never fired",
            m.p1().character.state_no,
            m.p1().character.ctrl,
            m.p1().character.move_type,
        );
    }

    /// T056 regression (AC1, bug "flies off screen"): a human mashing a long,
    /// varied sequence of inputs at a live evilken must never drive its position
    /// to a non-finite value or far outside the stage. This is a stability guard
    /// against a velocity/position blow-up (NaN, or runaway off-screen) from any
    /// state the synthesized input sequence reaches. Position must stay finite and
    /// well within an order of magnitude of the stage half-width.
    ///
    /// Asset-gated: skips cleanly when `test-assets/evilken/` is absent.
    #[test]
    fn evilken_human_input_mash_does_not_blow_up_position() {
        let Some(mut m) = build_evilken_match() else {
            return; // fixture absent -> skip cleanly
        };
        evilken_into_fight(&mut m);
        ...
        for i in 0..1200usize {
            let f = inputs[(i / 3) % inputs.len()];
            m.tick(f, MatchInput::none());
            let c = &m.p1().character;
            assert!(
                c.pos.x.is_finite() && c.pos.y.is_finite(),
                "evilken position became non-finite at tick {i}: pos=({}, {}) — a \
                 velocity/position blow-up (the 'flies off screen' bug)",
                c.pos.x,
                c.pos.y,
            );
            assert!(
                c.pos.x.abs() < 5_000.0 && c.pos.y.abs() < 5_000.0,
                "evilken position ran far off-screen at tick {i}: pos=({}, {}) \
                 (state={}, vel=({}, {})) — a velocity/position blow-up",
                c.pos.x,
                c.pos.y,
                c.state_no,
                c.vel.x,
