...
| **FNT** (`.fnt` fonts) | **Partial** | An FNT v1 parser now exists (`fnt.rs`) and `fp-render` has a `draw_text`/glyph path consumed by the screenpack HUD. FNT **v2** (MUGEN 1.0+ SFF sprite-font) is now detected and its glyph table parsed (`FntFont::inspect_v2` → `FntV2Info`), but not yet renderable — `from_bytes` falls back with `FpError::Unsupported` (audit **#T031**, see the FNT v2 row below). **Caveats:** asset-blocked (no real `.fnt` fixture — synthetic-tested), and the *legacy quad HUD* is not yet wired to it. Audit **#30**. |
...
| **FNT v2** sprite/TTF font (MUGEN 1.0+) | **Detected (parse-only)** | **Detected** via its version byte (`detect_fnt_version` → `FntVersion::V2`) and its `[Def]`/`[Map]` glyph table is **parsed** into `FntV2Info` (`FntFont::inspect_v2` — `char → SFF sprite index`, never panics). The SFF-backed sprite-font is not yet *renderable* through the bitmap `draw_text` path, so `FntFont::from_bytes` warns-and-returns `FpError::Unsupported` (a safe fallback to the bitmap HUD font; the error message carries the declared glyph count) — never a crash. Synthetic-tested. Audit **#T031**. `fnt.rs`. |
