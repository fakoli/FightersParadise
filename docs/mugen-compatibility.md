...
| **FNT** (`.fnt` fonts) | **Partial** | An FNT v1 parser now exists (`fnt.rs`) and `fp-render` has a `draw_text`/glyph path consumed by the screenpack HUD. FNT **v2** (MUGEN 1.0+ — a *text* `.def`-style font that references an external SFF/TTF, **not** a binary file) is **detected and reported `Unsupported`** (never decoded): `from_bytes` warns-and-falls-back to the bitmap HUD font, and `FntFont::inspect_v2` parses its `[Def]`/`[Map]` into `FntV2Info` for diagnostics. See the FNT v1/v2 rows in the variant matrix below (audit **#T031**). **Caveats:** asset-blocked (no real `.fnt` fixture — synthetic-tested), and the *legacy quad HUD* is not yet wired to it. Audit **#30**. |
...
| **FNT v2** sprite/TTF font (MUGEN 1.0+) | **Detected (parse-only)** | FNT v2 is **not** a binary file — it is a **text `.def`-style** font (no `ElecbyteFnt\0` magic) with a `[Def]` (`type = bitmap`/`truetype`, `file`, `size`, `spacing`, `offset`, `blend`) that references an external **SFF** sprite-font (bitmap) or `.ttf` (truetype); a bitmap `[Map]` lists glyphs as `0,<ASCII>` (group, then the character's ASCII code). It is **detected** as such by `detect_fnt_version` → `FntVersion::V2` (distinguished from the v1 binary header by recognising the leading `[Def]`/`type=` text body, not a version byte), and `FntFont::inspect_v2` **parses** the `[Def]`/`[Map]` into `FntV2Info` (`type`, referenced `file`, and `char → SFF sprite index`, never panics). The SFF/TTF-backed font is not yet *renderable* through the bitmap `draw_text` path, so `FntFont::from_bytes` warns-and-returns `FpError::Unsupported` (a safe fallback to the bitmap HUD font; the message carries the font type + declared glyph count) — never a crash. Synthetic-tested (text-shaped fixture). Audit **#T031**. `fnt.rs`. |
...
| **Screenpack / Lifebar** (`system.def`/`fight.def`) | **Partial** | Typed model + HUD render; `[Combo]`/`[Face]` parsed-not-drawn, single bg layer. Player-side **HUD customization** (life/power-bar color presets + per-element visibility) layers on top via `fp_ui::HudConfig`. `fp-ui`. |
| **Directory-based content discovery** (`chars/`, `stages/`, `data/<motif>/`) | **Supported** | MUGEN's by-convention content layout is auto-discovered, no index file needed: a character roster from `chars/<name>/<name>.def` (or a flat dir of `*.def`) via `fp_ui::discovery::discover_chars`; stages from a `stages/` folder (each `*.def` validated to carry a stage section) via `fp_stage::discover_stages`; and screenpack **motif** sets from `data/<motif>/system.def` via `fp_ui::discovery::discover_motifs`. Point `fp-app` at a directory to launch the Title menu over the discovered roster; pick a motif with the `--motif <name\|path>` flag (a discovered name, a `system.def` path, or a motif dir). Each scan is deterministic (case-insensitive sort) and never-crash (a bad dir warns and yields an empty list). `fp-ui/discovery.rs`, `fp-stage/lib.rs`, `fp-app/main.rs`. |
