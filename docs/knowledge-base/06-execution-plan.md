...
| 4.10 | DOING | **Real-content trigger support** | `fp-vm` | Close the 4 gaps 4.6 found in kfm.cns (→ ~99% clean parse): (1) axis-suffix triggers `Vel Y`/`Pos X`/`P2BodyDist X` (~65), (2) `AnimElem = N, op M` comma-tail, (3) dotted call args `GetHitVar(fall.yvel)`, (4) **`command="x"` string-equality** (currently evals 0 → moves never fire; needs an `EvalContext` string/command query). Update cns_integration thresholds. | 4.6 |
