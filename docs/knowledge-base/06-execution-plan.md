...
| 4.6 | DOING | **Integration:** kfm.cns → lex/parse/eval (keystone validation) | `fp-vm`/tests (+`fp-formats` dev-dep) | Load real kfm.cns+common1.cns; tokenize+parse EVERY trigger string (assert no panic; tally clean vs recoverable-error — `:=` forms expected to defer to 4.9); evaluate a curated sample against MockContext with expected results. Skip-if-missing. | 4.4, 4.5, 4.8, 0.3 |
