...
| 6.2c | DOING | **Facing-relative velocity** (executor) + drop walk repair | `fp-character`+`fp-app` | MUGEN integrates world `pos.x += vel.x * facing` (vel stored facing-relative; `Vel X` trigger returns facing-relative). Fix the executor physics to mirror x by facing on integration (+ `PosAdd` x); then REMOVE the fp-app `inject_engine_movement_bridge` walk-velocity repair (CB26) — facing-left walk must work from the executor, not the shim. *(atomic 2-crate — done via direct agent + review since the executor change + repair removal must land together)* | 6.2b |
